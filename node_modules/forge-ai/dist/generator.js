...
class Generator {
    ...
    async generate(prompt, config = {}) {
        const { maxTokens = 50, temperature = 0.7, topK = 50, topP = 0.9, repetitionPenalty = 1.1, stop = [], } = config;
        // Encode the prompt
        const inputTokens = this.tokenizer.encode(prompt, true, false);
        // Clear KV cache for fresh generation
        this.model.clearCache();
        // Generate using the model
        const outputTokens = this.model.generate(inputTokens, maxTokens, { temperature, topK, topP, repetitionPenalty });
    ...
    async *stream(prompt, config = {}) {
        ...
        for (let i = 0; i < maxTokens; i++) {
            ...
            // Sample next token
            const nextToken = this._sampleFromLogits(logits, tokens, temperature, topK, topP, repetitionPenalty);
            // Check for EOS
            if (nextToken === EOS_ID)
                break;
            // Add to sequence
            ...
            if (stop.some(s => generated.includes(s)))
                break;
        }
    }
    /**
     ...
     */
    _sampleFromLogits(logits, previousTokens, temperature, topK, topP, repetitionPenalty) {
